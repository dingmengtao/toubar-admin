...
class NavigationDataTable extends AbstractDataTables
{
    /**
     ...
     * @var string
     */
    protected $screenName = 'webed-blog';

    public function __construct()
    {
        $this->model = do_filter(FRONT_FILTER_DATA_TABLES_MODEL,
            Navigation::select(['id', 'title', 'slug','created_at']),
            $this->screenName
        );
    }
    ...
    public function headings(): array
    {
        return [
            ...
            'slug' => [
                'title' => 'slug',
                'slug' => trans('webed-core::datatables.heading.slug'),
                'width' => '10%',
            ],
            'created_at' => [
    ...
    {
   $this->setAjaxUrl(route('admin::blog.navigation.index.post'),'POST');
...
//            })
            ->addColumn('actions', function ($item) {
                /*Edit link*/
                $activeLink = route('admin::blog.navigation.update-status.post', ['id' => $item->id, 'status' => 1]);
                $disableLink = route('admin::blog.navigation.update-status.post', ['id' => $item->id, 'status' => 0]);
                $deleteLink = route('admin::blog.navigation.delete.post', ['id' => $item->id]);

                /*Buttons*/
                $editBtn = link_to(route('admin::blog.navigation.edit.get', ['id' => $item->id]), trans('webed-core::datatables.edit'), ['class' => 'btn btn-sm btn-outline green']);
        ...
        if ($request->input('customActionType', null) === 'group_action') {
            ...
            switch ($actionValue) {
                case 'deleted':
                    ...
                    $action = app(UpdateNavigationAction::class);
