...
class TagsListDataTable extends AbstractDataTables
{
    ...
    protected $screenName = WEBED_BLOG_TAGS;

    public function __construct()
    {
        $this->model = do_filter(
            FRONT_FILTER_DATA_TABLES_MODEL,
            BlogTag::select('title', 'status', 'slug', 'order', 'created_at', 'id', 'deleted_at')->withTrashed(),
            $this->screenName
        );
    }
    ...
    protected function fetchDataForAjax()
    {
        return webed_datatable()->of($this->model)
            ...
            ->addColumn('actions', function ($item) {
                /*Edit link*/
                $activeLink = route('admin::blog.tags.update-status.post', ['id' => $item->id, 'status' => 1]);
                $disableLink = route('admin::blog.tags.update-status.post', ['id' => $item->id, 'status' => 0]);
                $deleteLink = route('admin::blog.tags.delete.post', ['id' => $item->id]);
                $forceDelete = route('admin::blog.tags.force-delete.post', ['id' => $item->id]);
                $restoreLink = route('admin::blog.tags.restore.post', ['id' => $item->id]);
    ...
    protected function groupAction(): array
    {
        ...
        if ($request->input('customActionType', null) === 'group_action') {
            if (!has_permissions(get_current_logged_user(), ['edit-tags'])) {
                return [
            ...
            switch ($actionValue) {
                case 'deleted':
                    if (!has_permissions(get_current_logged_user(), ['delete-tags'])) {
                        return [
                            'customActionMessage' => trans('webed-acl::base.do_not_have_permission'),
                            'customActionStatus' => 'danger',
                        ];
                     ...
                     * Delete items
                     */
                    $action = app(DeleteTagAction::class);
                    foreach ($ids as $id) {
                        $action->run($id);
                    }
                    break;
                case 0:
                case 1:
                    $action = app(UpdateTagAction::class);
